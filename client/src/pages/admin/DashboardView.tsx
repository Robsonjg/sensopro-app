...
import { Download, Users, BarChart3, FlaskConical, RotateCcw } from "lucide-react";
...
export default function DashboardView({ experimentoId, onSelectExp }: Props) {
  ...
  const limparMut = trpc.dashboard.limparResultados.useMutation({
    onSuccess: async () => {
      await utils.dashboard.getData.invalidate({ experimento_id: activeId! });
      await utils.dashboard.exportar.invalidate({ experimento_id: activeId! });
      toast.success("Resultados limpos. O experimento está pronto para começar do zero.");
    },
  ...
  const chartData = useMemo(() => {
    ...
    const porAtributo = atributos.map((attr: any) => {
      ...
      amostras.forEach((am: any) => {
        const m = medias.find((x: any) => x.atributo_id === attr.id && x.amostra_id === am.id);
    ...
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          <Button
            onClick={() => {
              if (!activeId || !data?.total || limparMut.isPending) return;
              ...
              if (ok) limparMut.mutate({ experimento_id: activeId });
            }}
            disabled={!activeId || !data?.total || limparMut.isPending}
            variant="outline"
            style={{ gap: '8px', borderRadius: '9999px', color: '#b91c1c', borderColor: '#fecaca' }}
          >
            <RotateCcw size={16} />
            {limparMut.isPending ? "Limpando…" : "Limpar resultados"}
          </Button>
        </div>
      </div>
