...
import { admins, amostras, atributos, convites, experimentos, respostas, sessoes, users, } from "../drizzle/schema.js";
...
// ─── Experimentos ─────────────────────────────────────────────────────────────
export async function listExperimentos(adminId) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select().from(experimentos).orderBy(desc(experimentos.criadoEm));
}
export async function getExperimentoById(id) {
    const db = await getDb();
    if (!db)
        return undefined;
...
export async function getExperimentoBySlug(slug) {
    const db = await getDb();
    if (!db)
        return undefined;
...
export async function createExperimento(data) {
    const db = await getDb();
    if (!db)
...
export async function updateExperimento(id, data) {
    const db = await getDb();
    if (!db)
        return;
    await db
...
export async function deleteExperimento(id) {
    const db = await getDb();
    if (!db)
        return;
    await db.delete(experimentos).where(eq(experimentos.id, id));
}
export async function ativarExperimento(id) {
    const db = await getDb();
    if (!db)
        return;
...
export async function desativarExperimento(id) {
    const db = await getDb();
    if (!db)
        return;
...
// ─── Amostras ─────────────────────────────────────────────────────────────────
export async function listAmostras(experimentoId) {
    const db = await getDb();
    if (!db)
        return [];
    return db
...
export async function createAmostra(data) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not connected");
...
export async function updateAmostra(id, data) {
    const db = await getDb();
    if (!db)
        return;
...
// ─── Atributos ────────────────────────────────────────────────────────────────
export async function listAtributos(experimentoId) {
    const db = await getDb();
    if (!db)
        return [];
    return db
...
export async function createAtributo(data) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not connected");
...
export async function updateAtributo(id, data) {
    const db = await getDb();
    if (!db)
        return;
    await db.update(atributos).set(data).where(eq(atributos.id, id));
}
export async function deleteAtributo(id) {
    const db = await getDb();
    if (!db)
        return;
...
export async function getSessaoExistente(experimentoId) {
    const db = await getDb();
    if (!db)
        return undefined;
...
export async function finalizarSessao(sessaoId, tempoTotal, finalizadoEm) {
    const db = await getDb();
    if (!db)
        return;
...
// ─── Respostas ────────────────────────────────────────────────────────────────
export async function upsertResposta(data) {
    const db = await getDb();
    if (!db)
        return;
    await db
...
export async function listRespostasBySessao(sessaoId) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select().from(respostas).where(eq(respostas.sessaoId, sessaoId));
}
export async function getRespostasCompletas(experimentoId) {
    const db = await getDb();
    if (!db)
        return [];
    return db
...
export async function getDashboardData(experimentoId) {
    const db = await getDb();
    if (!db) {
...
export async function promoteAdminByEmail(email) {
    const db = await getDb();
    if (!db)
        return undefined;
    const result = await db
...
export async function deactivateAdminByEmail(email) {
    const db = await getDb();
    if (!db)
        return undefined;
    const result = await db
...
export async function getConviteByCode(codigo) {
    const db = await getDb();
    if (!db)
        return undefined;
